...
func addRunnerAuthMiddleware(b *backend) gin.HandlerFunc {
	return func(c *gin.Context) {
		authHeader := c.GetHeader("Authorization")
		if authHeader == "" {
			c.JSON(http.StatusUnauthorized, gin.H{
		...
		claims, err := util.ValidateJWT(parts[1], b.runnerJwt.secret)
		if err != nil {
			c.JSON(http.StatusUnauthorized, gin.H{
				"message": "Invalid token: " + err.Error(),
			})
		...
		if claims[constant.USER_LEVEL_CLAIM_TAG] != constant.USER_LEVEL_RUNNER {
			c.JSON(http.StatusForbidden, gin.H{
